...
%token COMMENT PROC MAIN BEGN COLON END VAR_DECL INT CHAR FLOAT DOUBLE WRITE READ FOR TO OF DO ENDFOR IF ELSEIF ELSE ENDIF GOTO REPEAT UNTIL WHILE ENDWHILE ARROW LB RB SC COMA ASSIGNMENT LAND LOR GEQ LEQ GT LT NEQ DEQ PLUS MINUS MUL DIV MOD NOT TEXT VAR NUMBER ARRAY
...
prm: PROC MAIN LB RB BEGN COLON { printf("#include<stdio.h>\nint main()\n{\n");  }
     PROG_STATEMENTS END COLON { printf("\n}"); }

PROG_STATEMENTS: PROG_STATEMENTS PROG_STATEMENT | ;

PROG_STATEMENT: VAR_DECL VAR_LIST COLON DATA_TYPE SC {
              ...
              | WRITE LB { printf("printf(\""); } TO_WRITE RB SC {
                printf("\"");
                for(int i=0; i<idx; i++)
                {
                  printf(",%s", var_list[i]);
              ...
              |GOTO { printf("goto "); }  VAR { printf("%s", yylval.var_name); } SC { printf(";"); }
